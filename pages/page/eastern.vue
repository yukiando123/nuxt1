<template>
<div class="bg-white py-6 sm:py-8 lg:py-12">
    <div class="max-w-screen-2xl px-4 md:px-8 mx-auto">
        <div class="mb-10 md:mb-16">
            <h2 class="text-gray-800 text-2xl lg:text-3xl font-bold text-center mb-4 md:mb-6">スタジアムバーサイト</h2>
            <!--<p class="max-w-screen-md text-gray-500 md:text-lg text-center mx-auto">This is a section of some simple filler text, also known as placeholder text. It shares some characteristics of a real written text but is random or otherwise generated.</p>-->
        <div class="max-w-screen-lg px-4 md:px-8 mx-auto">
        <div class="text-gray-800 text-lg font-semibold mb-3">Description</div>
            <p class="text-gray-500">
            横浜スタジアム目の前にオープンしたスポーツバーの公式サイトおよび店内システム
            </p>
        </div>
        </div>
        </div>

<div class="max-w-screen-lg px-4 md:px-8 mx-auto">
    <table class="table-auto text-left w-full">
    <thead>
        <tr>
            <th class="border px-4 py-2">タイトル</th>
            <td class="border px-4 py-2">STBバー</td>
        </tr>
    </thead>
    <tbody>
        <tr>
            <th class="border px-4 py-2">クライアント</th>
            <td class="border px-4 py-2">株式会社JTIS</td>
        </tr>
        <tr>
            <th class="border px-4 py-2">リンク</th>
            <td class="border px-4 py-2">https://stb.bar/</td>
        </tr>
        <tr>
            <th class="border px-4 py-2">Scale</th>
            <td class="border px-4 py-2">社内メンバー3名</td>
        </tr>
        <tr>
            <th class="border px-4 py-2">担当</th>
            <td class="border px-4 py-2">
            サイトデザイン、機能開発（スタッフ管理、イベント情報）
            </td>
        </tr>
        <tr>
          <th class="border px-4 py-2">使用ツール</th>
          <td class="border px-4 py-2">
            Illustrator, VScode
        </td>
        </tr>
        <tr>
            <th class="border px-4 py-2">技術スタック</th>
            <td class="border px-4 py-2">PHP, HTML, CSS, JavaScript</td>
        </tr>
        </tbody>
    </table>
    </div>

        <div class="bg-white py-6 sm:py-8 lg:py-12">
    <div class="max-w-screen-lg px-4 md:px-8 mx-auto">
        <div class="overflow-hidden mb-5 shadow-lg">
            <img src="~/assets/projects/eastern.jpg" loading="lazy" alt="Photo by Fakurian Design">
        </div>
    </div>
</div>
    </div>
</div>
</template>

<script>
export default {
    layout: 'sub'
}
</script>

<style>

</style>
<template>
<div class="bg-white py-6 sm:py-8 lg:py-12">
    <div class="max-w-screen-2xl px-4 md:px-8 mx-auto">
        <div class="mb-10 md:mb-16">
            <h2 class="text-gray-800 text-2xl lg:text-3xl font-bold text-center mb-4 md:mb-6">CRAZY CAT</h2>
            <!--<p class="max-w-screen-md text-gray-500 md:text-lg text-center mx-auto">This is a section of some simple filler text, also known as placeholder text. It shares some characteristics of a real written text but is random or otherwise generated.</p>-->
        <div class="max-w-screen-lg px-4 md:px-8 mx-auto">
        <div class="text-gray-800 text-lg font-semibold mb-3">Description</div>
            <p class="text-gray-500">
            出張マッサージスパCRAZY CAT<br>
            出張マッサージ「CRAZY CAT」（クレイジーキャット）は23区中心地をメインに出張するデリバリースパの公式サイト。
            </p>
        </div>
        </div>
        </div>

<div class="max-w-screen-lg px-4 md:px-8 mx-auto">
    <table class="table-auto text-left w-full">
    <thead>
        <tr>
            <th class="border px-4 py-2">タイトル</th>
            <td class="border px-4 py-2">クレイジーキャット</td>
        </tr>
    </thead>
    <tbody>
        <tr>
            <th class="border px-4 py-2">クライアント</th>
            <td class="border px-4 py-2">東京リラクゼーション株式会社</td>
        </tr>
        <tr>
            <th class="border px-4 py-2">リンク</th>
            <td class="border px-4 py-2">https://ccat.jp</td>
        </tr>
        <tr>
            <th class="border px-4 py-2">Scale</th>
            <td class="border px-4 py-2">2,000,000~, 社内メンバー2名</td>
        </tr>
        <tr>
            <th class="border px-4 py-2">担当</th>
            <td class="border px-4 py-2">
            プロジェクトリーダー、要件定義、サイトデザイン、サイトコーディング、機能開発(WordPress：スタッフ管理、シフト管理等)
            </td>
        </tr>
        <tr>
          <th class="border px-4 py-2">使用ツール</th>
          <td class="border px-4 py-2">
            Illustrator, Figma, github, VScode
        </td>
        </tr>
        <tr>
            <th class="border px-4 py-2">技術スタック</th>
            <td class="border px-4 py-2">PHP, HTML, SCSS(FLOCSS設計), JavaScript, WordPress</td>
        </tr>
        </tbody>
    </table>
    </div>


    <div class="bg-white py-6 sm:py-8 lg:py-12">
    <div class="max-w-screen-lg px-4 md:px-8 mx-auto">
        <div class="bg-gray-100 overflow-hidden shadow-lg">
            <img src="~/assets/projects/ccat-sp.jpg" loading="lazy" alt="Photo by Himanshu Dewangan" class="w-full h-full object-cover object-center" />
        </div>

      <div class="grid md:grid-cols-2 gap-8">
        <div class="space-y-4">
        <div class="bg-gray-100 overflow-hidden shadow-lg">
            <img src="~/assets/projects/ccat-sp.jpg" loading="lazy" alt="Photo by Himanshu Dewangan" class="w-full h-full object-cover object-center" />
        </div>
      </div>

        <div class="space-y-4">
          <h3>mockup 2</h3>
          <div class="bg-gray-100 overflow-hidden shadow-lg">
              <img src="~/assets/projects/hanto3.jpg" loading="lazy" alt="Photo by Himanshu Dewangan" class="w-full h-full object-cover object-center" />
          </div>
      </div>
    </div>
</div>

    </div>
</div>
</template>

<script>
export default {
    layout: 'sub'
}
</script>

<style>

</style>
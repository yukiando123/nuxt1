<template>
<div class="bg-white py-6 sm:py-8 lg:py-12">
    <div class="max-w-screen-2xl px-4 md:px-8 mx-auto">
        <div class="mb-10 md:mb-16">
            <h2 class="text-gray-800 text-2xl lg:text-3xl font-bold text-center mb-4 md:mb-6">KUROFUNE NFTマーケットプレイス</h2>
            <!--<p class="max-w-screen-md text-gray-500 md:text-lg text-center mx-auto">This is a section of some simple filler text, also known as placeholder text. It shares some characteristics of a real written text but is random or otherwise generated.</p>-->
        <div class="max-w-screen-lg px-4 md:px-8 mx-auto">
        <div class="text-gray-800 text-lg font-semibold mb-3">Description</div>
            <p class="text-gray-500">
KUROFUNE NFTマーケットプレイス
            </p>
        </div>
        </div>
        </div>

<div class="max-w-screen-lg px-4 md:px-8 mx-auto">
    <table class="table-auto text-left w-full">
    <thead>
        <tr>
            <th class="border px-4 py-2">タイトル</th>
            <td class="border px-4 py-2">KUROFUNE NFTマーケットプレイス</td>
        </tr>
    </thead>
    <tbody>
        <tr>
            <th class="border px-4 py-2">クライアント</th>
            <td class="border px-4 py-2">株式会社TORAT</td>
        </tr>
        <tr>
            <th class="border px-4 py-2">Link</th>
            <td class="border px-4 py-2">https://kurofune.world/</td>
        </tr>
        <tr>
            <th class="border px-4 py-2">Scale</th>
            <td class="border px-4 py-2">社内メンバー2名, オフィショア開発2名</td>
        </tr>
        <tr>
            <th class="border px-4 py-2">担当</th>
            <td class="border px-4 py-2">
            サイト設計、サイトデザイン、サイトフロントコーディング
            </td>
        </tr>
        <tr>
          <th class="border px-4 py-2">開発ツール</th>
          <td class="border px-4 py-2">
            XD, VScode
          </td>
        </tr>
        <tr>
            <th class="border px-4 py-2">技術スタック</th>
            <td class="border px-4 py-2">React, Next.js, Solidity</td>
        </tr>
        <tr>
            <th class="border px-4 py-2">Features</th>
            <td class="border px-4 py-2">Ethereum chain, EVM, ERC721</td>
        </tr>
        </tbody>
    </table>
    </div>

</template>

<script>
export default {
    layout: 'sub'
}
</script>

<style>

</style>